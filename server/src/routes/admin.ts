...
AdminRouter.post('/upload-student', async (req : Request, res : Response) =>{
    createStudentWithXlsxFile(process.env.FILE_PATH as string)
        .then(() => {
            return res.status(200).send({ success: true, msg: "Les étudiants ont été ajoutés avec succès" });
        })
        .catch((error) => {
            return res.status(400).send({ success: false, msg: "Une erreur s'est produite veuillez contactez nos développeurs" });
        })
})